...
func _init() -> void:
	...
	for index in range(40):
		var lane: int = scheduler.call("next_lane", rng); both[lane] = true; streak = streak + 1 if lane == previous else 1; _check(streak <= 2, "lane fairness %d" % index); previous = lane
	...
	for id in [&"hunger", &"rest", &"fun"]:
		var boost: ItemDefinition = Item.new(); var boost_deltas: Dictionary[StringName, float] = {}; boost_deltas[id] = 7.0; boost.deltas = boost_deltas; simple.append(boost)
		var contextual: ItemDefinition = Item.new(); var contextual_deltas: Dictionary[StringName, float] = {}; contextual_deltas[id] = 8.0; contextual.is_tradeoff = true; contextual.deltas = contextual_deltas; trade.append(contextual)
	var bag: Array[ItemDefinition] = Generator.new().call("generate", simple, trade, Rng.new(99))
	var trade_count := 0
	...
	var drought_item = Item.new(); var drought_deltas: Dictionary[StringName, float] = {}; drought_deltas[&"hunger"] = -1.0; drought_deltas[&"rest"] = 1.0; drought_deltas[&"fun"] = 1.0; drought_item.deltas = drought_deltas
