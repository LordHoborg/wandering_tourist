...
The tropical prototype introduces its vocabulary over three sequential stages while preserving the approved 20-80 survival bounds, item effects, lane controls, cut window, and fairness rules. Stage 1 is 65 seconds at -0.49/second with Fruit, Pillow, and Camera only; it completes early after two collections of each essential. Stage 2 is 85 seconds at -0.48/second and introduces Stale Snack, Alarm Clock, and Rain Cloud one at a time through controlled three-item teaching bags; it completes after two collections of each essential and one correct pass of each hazard. Stage 3 is the 100-second full challenge at -0.40/second, restoring the approved seven-simple/three-trade bag and adding Coffee, Local Meal, and Night Market with compact signed H/R/F effects. Spawn intervals and fall durations are stage configuration: 1.50/3.45, 1.35/3.10, and 1.20/2.90 seconds respectively.

Item familiarity is profile-local for the active campaign and per item ID: the first encounter is NEW, encounters two through five are LEARNING, and later encounters are KNOWN. Simple-item action labels are not permanent; early exposure shows effects, while trade-offs always show their signed effects. Travel Spirit is a non-survival performance meter: correct simple cuts, hazard passes, and contextually beneficial trade-offs build it and grant a small score bonus; missed beneficial items and poor choices reduce it. Completion results report decision statistics, rating, and N advances to the next stage; R retries only the current stage.
