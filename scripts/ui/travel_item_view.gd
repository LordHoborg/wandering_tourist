...
func configure(data: Dictionary) -> void:
	item_id = data["id"]; collect = data["collect"]; tradeoff = data["tradeoff"]; cut_ready = data["cut_ready"]
...
func _draw() -> void:
	...
	draw_string(ThemeDB.fallback_font, Vector2(77, 38), _title(), HORIZONTAL_ALIGNMENT_LEFT, 88, 15, Color.WHITE); draw_string(ThemeDB.fallback_font, Vector2(77, 61), "MIXED" if tradeoff else ("TAKE" if collect else "AVOID"), HORIZONTAL_ALIGNMENT_LEFT, -1, 12, accent)
