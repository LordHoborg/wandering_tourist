...
func _draw() -> void:
	...
	var action_text: String = "SPACE TO RESUME    R TO RESTART" if state == RunStateMachine.State.PAUSED else (continue_text if state == RunStateMachine.State.COMPLETED and snapshot.get("stage", 1) < 3 else "PRESS R TO START A NEW TRIP")
